    ...
    switch (node->type)
    {
        case SPECIAL_TYPE:
        {
            ...
            switch (node->value.l_type)
            {
                case ST:
                    Asm_Node(LEFT_C);
                    if (IS_NOT_NIL(RIGHT_C)) Asm_Node(RIGHT_C);
                    break;
                case FUNC:
                    Asm_Node(LEFT_C);
                    Asm_Node(RIGHT_C);
                    break;
                case RET:
                    if (IS_NOT_NIL(LEFT_C)) Asm_Node(LEFT_C);
                    PRINT_ASM("pop [999]");
                    PRINT_ASM("RET");
                    break;
                case PARAM:
                    ...
                    {
                        Node* cur_node = node;
                        while (IS_NOT_NIL(cur_node->right))
                        {
                            //printf("while2\n");
                            PRINT_ASM("in");
                    ...
                    {
                        Node* cur_node = node;
                        //printf("IS NOT SUKA NIL %d\n", IS_NOT_NIL(cur_node->right));
                        while (IS_NOT_NIL(cur_node->right))
                        {
                            //printf("while1\n");
                            //printf("in while!\n");
                ...
                case IF:
                    if (node->right->value.l_type == ST)
                    {
                        ...
                        IF_LABELS++;
                        int b = IF_LABELS;
                        Asm_Condition(node->left, false, a, b, false);
                ...
                case WHILE:
                {
                    ...
                    PRINT_ASM("if_label_%d:", a);
                    Asm_Condition(node->left, true, a, b, true);
                    Asm_Node(node->right);
                ...
                case CALL:
                {
                    Node* name_node = node->left;
                    if(IS_NOT_NIL(name_node->left)) Asm_Node(name_node->left);
                    PRINT_ASM("call %s", name_node->value.name);
                    PRINT_ASM("push[999]");
                    break;
                }
        ...
        {
            //printf("value is %lf\n", node->value.dval);
            //PRINT_ASM("push %lf", node->value.dval);
            PRINT_ASM("\npush %d", (int)(node->value.dval*ACCURACY));
            PRINT_ASM("push %d", ACCURACY);
            PRINT_ASM("div\n");
            break;
        }
...
void Asm_Condition(Node* node, bool reversed, int begin_if, int begin_else, bool While)
{
    ...
    case OR:
    {
        ...
        IF_LABELS++;
        Asm_Condition(node->left, reversed, end_or, begin_else, While);
        Asm_Condition(node->right, reversed, end_or, begin_else, While);
    ...
    case AND:
    {
        ...
        IF_LABELS++;
        Asm_Condition(node->left, !reversed, begin_if, end_and, While);
        Asm_Condition(node->right, !reversed, begin_if, end_and, While);
        PRINT_ASM("jmp if_label_%d", begin_if);
    ...
    case IS_EE:
    {
        ...
        if (!reversed)
        {
            ...
            if(!While) PRINT_ASM("jmp if_label_%d", begin_else);
        }
        else
        {
            PRINT_ASM("jne id_label_%d", begin_else);
            if(!While) PRINT_ASM("jmp if_label_%d", begin_if);
        }
        break;
    }
